...
<div class="email_subscription block_newsletter" id="blockEmailSubscription_{$hookName}">
  ...
  <form action="{$urls.current_url}#blockEmailSubscription_{$hookName}" method="post">
    <input type="email" name="email" value="{$value}" placeholder="{l s='Your birthday' d='Modules.Emailsubscription.Shop'}" required />
    <input type="date" name="birthday" value="" placeholder="{l s='Your birthday' d='Modules.Emailsubscription.Shop'}" />
    {if $conditions}
      <p>{$conditions}</p>
    {/if}
