...
class Ps_Emailsubscription extends Module implements WidgetInterface
{
    ...
    public function renderList()
    {
        $fields_list = array(
            ...
            'birthday' => array(
                'title' => $this->trans('Data di nascita', array(), 'Modules.Emailsubscription.Admin'),
                'type' => 'date',
                'search' => false,
            ),
        );
    ...
    {
        $dbquery = new DbQuery();
        $dbquery->select('c.`id_customer` AS `id`, s.`name` AS `shop_name`, gl.`name` AS `gender`, c.`lastname`, c.`firstname`, c.`email`, c.`newsletter` AS `subscribed`, c.`newsletter_date_add`, l.`iso_code`, c.`birthday`');
        ...
        $dbquery = new DbQuery();
        $dbquery->select('CONCAT(\'N\', e.`id`) AS `id`, s.`name` AS `shop_name`, NULL AS `gender`, NULL AS `lastname`, NULL AS `firstname`, e.`email`, e.`active` AS `subscribed`, e.`newsletter_date_add`, l.`iso_code`, e.`birthday`');
    ...
    protected function registerUser($email)
    {
        ...
        /* Fabio: l'ho aggiunto io */
        $birthday = addslashes($_POST['birthday']);

        $sql = 'UPDATE ' . _DB_PREFIX_ . 'customer
                SET `newsletter` = 1, newsletter_date_add = NOW(), `ip_registration_newsletter` = \'' . pSQL(Tools::getRemoteAddr()) . '\', `birthday` = \'' . $birthday . '\'
    ...
    protected function registerGuest($email, $active = true)
    {
        ...
        $sql = 'INSERT INTO ' . _DB_PREFIX_ . 'emailsubscription (id_shop, id_shop_group, email, newsletter_date_add, ip_registration_newsletter, http_referer, active, id_lang, birthday)
                VALUES
